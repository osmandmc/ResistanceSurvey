...
Big picture
- ASP.NET Core MVC app with EF Core for the main data access layer (`RS.EF/RSDBContext.cs`) and a Dapper-based UnitOfWork/DAL (`RS.DAL/UnitOfWork.cs`) used in places for lightweight operations.
...
- Some code uses a Dapper `UnitOfWork` (`RS.DAL/UnitOfWork.cs`) which opens a raw SqlConnection and begins a transaction. Prefer using EF services unless the task explicitly targets performance/SQL/legacy DAL.
...
- Named lookups: simple lookup tables (City, District, Category, etc.) are exposed via `ILookupRepository` / `IUnitOfWork.LookupRepository`. Prefer those helpers for UI list population.
...
- Dapper UnitOfWork: `RS.DAL/UnitOfWork.cs` and repositories under `RS.DAL/Repositories` for raw SQL patterns and transaction management.
...
- When introducing schema changes, update both EF entities and any Dapper SQL in `RS.DAL` that reference table/column names.
